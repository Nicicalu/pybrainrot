...
from pybrainrot import BRAINROT2PY_MAPPINGS

"""
Python module for converting pybrainrot code to python code.
"""


def _ends_in_brainrot(word):
    """
    Returns True if word ends in .brainrot, else False

    Args:
        word (str):     Filename to check

    Returns:
        boolean: Whether 'word' ends with 'brainrot' or not
    """
    return word[-9:] == ".brainrot"


def _change_file_name(name, outputname=None):
    """
    Changes *.brainrot filenames to *.py filenames. If filename does not end in .brainrot,
...
def parse_glazes(filename):
    ...
    of all the imported modules (ie, module name appended with ".brainrot")

    Args:
        filename (str):     Path to file

    Returns:
        list of str: All imported modules, suffixed with '.brainrot'. Ie, the name
        the imported files must have if they are pybrainrot files.
    """
    ...
    glazes_with_suffixes = [im + ".brainrot" for im in glazes + glazes2]
...
def safe_substitute(value, deescaped_key, line):
    """
    Performs Pybrainrot token substitution on a line, but ignores tokens inside of strings.
    TODO: Can be extended to ignore tokens inside of comments as well.

    Args:
        value (str):             Python token
        deescaped_key (str):     Pybrainrot token
        line (str):              Code line

    Returns:
        Code line with safe Pybrainrot token substitutions
    """
    string_pattern = r"""
...
def parse_file(filepath, filename_prefix, outputname=None, change_imports=None):
    """
    Converts a pybrainrot file to a python file and writes it to disk.

    Args:
        filename (str):             Path to the pybrainrot file you want to parse.
        ...
        outputname (str):           Optional. Override name of output file. If
                                    omitted it defaults to substituting '.brainrot' to
                                    '.py'
        change_imports (dict):      Names of imported pybrainrot modules, and their
                                    python alternative.
    """
    ...
    for line in infile_str_raw.split("\n"):
        ...
        for key, value in BRAINROT2PY_MAPPINGS.items():
